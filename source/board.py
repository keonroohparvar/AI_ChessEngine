...
class ChessBoard():
    def __init__(self, name):
        self.name = name
        self.state = [[0, 0, 0, 0, 0, 0, 0, 0] for _ in range(8)]
        self.counter = 0
        self.enc = ''

        # Calls initialize board to start the board
        self.init_board()
    
    def init_board(self):
        """
        Creates initial board implementation.        
        """
        # Add Pieces
        pieces = self.create_all_pieces()

        # Populate board
        self.populate_board(pieces)
    ...
    def populate_board(self, all_pieces):
        for i in range(len(self.state)):
            for j in range(len(self.state[i])):
                for itr in range(len(all_pieces)):
                    if all_pieces[itr].position == [i, j]:
                        self.state[i][j] = all_pieces[itr]
    ...
    def make_move(self, move):
        player_to_move = 'w' if (self.counter % 2 == 0) else 'b'
    ...
    def find_piece_to_move(self, move):
        ...
        # Pawn
        if piece_char == piece_char.lower():
